...
struct WindowInvalidatorInner {
    ...
    /// Platform hook invoked on every clean->dirty transition so a
    /// demand-driven frame pump (e.g. the Windows vsync thread) can wake.
    /// `None` on platforms whose pump is not demand-driven.
    pub platform_waker: Option<Arc<dyn Fn() + Send + Sync>>,
}
...
impl WindowInvalidator {
    ...
    /// Install the platform's frame-pump waker; invoked on every dirty
    /// transition (and re-invoked by the frame callback for present-only
    /// demand). The waker must only touch thread-safe state.
    pub fn set_platform_waker(&self, waker: Arc<dyn Fn() + Send + Sync>) {
        self.inner.borrow_mut().platform_waker = Some(waker);
    }

    /// Wake the platform frame pump without marking the window dirty (used
    /// for present-only / callback-only frame demand).
    pub fn wake_platform(&self) {
        if let Some(waker) = self.inner.borrow().platform_waker.as_ref() {
            waker();
        }
    }
    ...
    fn record_frame_dirty(inner: &mut WindowInvalidatorInner) {
        // The single funnel for clean->dirty transitions: wake the platform
        // frame pump so a parked vsync thread schedules the next frame.
        if let Some(waker) = inner.platform_waker.as_ref() {
            waker();
        }
...
pub struct Window {
    ...
    /// Damage hint (logical pixels) for the next present, consumed by exactly
    /// one `present()`. `None` presents the full frame. See spec
    /// low-latency-present.
    present_damage: Option<Vec<Bounds<Pixels>>>,
...
impl Window {
    pub(crate) fn new(
        ...
        // Demand-driven frame pump (Windows): dirty transitions arm the
        // platform's per-window frame request.
        if let Some(waker) = platform_window.frame_pump_waker() {
            invalidator.set_platform_waker(waker);
        }
            ...
            move |request_frame_options| {
                ...
                if let Some(min_interval) = min_frame_interval {
                        ...
                        && now.duration_since(last_frame) < min_interval
                    {
                        ...
                        // Re-arm a demand-driven pump: this frame was skipped by
                        // throttling, so the pending work must get a later frame.
                        handle
                ...
                handle
                    .update(&mut cx, |_, window, _| {
                        window.complete_frame();
                        // Demand-driven pump: re-arm when this frame exits with
                        // outstanding demand (dirty again, present pending, or
                        // animation-frame callbacks queued during draw). Biased
                        // toward an extra frame over a missed one.
                    ...
                    .log_err();
                // Input-rate sustain also wants frames while active.
                if active.get() && input_rate_tracker.borrow_mut().is_high_rate() {
                    handle
                        .update(&mut cx, |_, window, _| window.invalidator.wake_platform())
                        .log_err();
                }
            }
        }));
...
impl Window {
    ...
    /// Hint the region (logical pixels) that changed since the last present,
    /// so the platform can pass dirty rects to the compositor. Consumed by
    /// exactly the next `present()`; not calling this presents the full frame.
    /// The full back buffer is still rendered — this is a composition hint
    /// only and never affects content correctness. See spec low-latency-present.
    pub fn set_present_damage(&mut self, rects: Vec<Bounds<Pixels>>) {
        self.present_damage = Some(rects);
    }
    ...
    #[profiling::function]
    fn present(&mut self) {
        // Consume this frame's damage hint; scale logical -> physical pixels
        // for the platform renderer (back-buffer coordinates).
        let damage: Option<Vec<Bounds<ScaledPixels>>> = self.present_damage.take().map(|rects| {
            rects
                .into_iter()
    ...
    /// Replace the already-painted backdrop inside `bounds` with a Gaussian-blurred
    /// copy of itself, rounded by `corner_radii`.
    ///
    /// Only content painted earlier in the frame is blurred, so this belongs before
    /// the element's own background. `blur_radius` follows the CSS `backdrop-filter:
    /// blur()` convention and is twice the Gaussian standard deviation. A radius at
    /// or below zero paints nothing.
    ///
    /// The primitive goes into the frame currently being built, so this only
    /// works while that frame is being painted.
    pub fn paint_backdrop_blur(
        ...
        self.next_frame.scene.insert_primitive(BackdropBlur {
            order: 0,
            sigma: (blur_radius / 2.).scale(scale_factor),
