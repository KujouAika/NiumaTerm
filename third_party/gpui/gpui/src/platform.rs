...
pub trait PlatformWindow: HasWindowHandle + HasDisplayHandle {
    ...
    /// A thread-safe waker that asks the platform's frame pump to schedule a
    /// frame for this window. Platforms with an unconditional (or
    /// compositor-driven) pump return `None` (the default): frames arrive
    /// without arming. A demand-driven pump (the Windows vsync thread) returns
    /// a waker that arms the window's frame request and wakes the pump.
    fn frame_pump_waker(&self) -> Option<std::sync::Arc<dyn Fn() + Send + Sync>> {
        None
    }
    ...
    /// Present a rendered scene. `damage` (physical pixels, back-buffer
    /// coordinates) hints which regions changed since the last present, for
    /// compositors that consume dirty rects; `None` presents the full frame.
    /// Platforms without dirty-rect support ignore it.
    fn draw(&self, scene: &Scene, damage: Option<&[Bounds<ScaledPixels>]>);
...
pub struct PathPromptOptions {
    ...
    /// File type filters to show when selecting files. Extensions may be written
    /// as `exe`, `.exe`, or `*.exe`.
    pub file_types: Vec<FileDialogFilter>,
}

/// One file type filter for a platform open-file dialog.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct FileDialogFilter {
    /// Human-readable filter name shown in the dialog.
    pub name: SharedString,
    /// File extensions included in this filter.
