...
#[allow(clippy::unnecessary_operation, clippy::identity_op)]
const _: () = {
    ["Size of BlockHandle"][::std::mem::size_of::<BlockHandle>() - 16usize];
    ["Alignment of BlockHandle"][::std::mem::align_of::<BlockHandle>() - 8usize];
    ["Offset of field: BlockHandle::id"][::std::mem::offset_of!(BlockHandle, id) - 0usize];
    ["Offset of field: BlockHandle::generation"]
        [::std::mem::offset_of!(BlockHandle, generation) - 8usize];
};
unsafe extern "C" {
    #[doc = " Finish the current command block.\n\n Freezes the primary screen into the block set and continues on a fresh\n primary screen. The caller is responsible for only invoking this at\n trusted command boundaries with the primary screen active (shell\n integration gating).\n\n         the active screen has no content (no block is created),\n         GHOSTTY_INVALID_VALUE when the alternate screen is active,\n         GHOSTTY_OUT_OF_MEMORY on allocation failure (terminal unchanged)\n"]
    pub fn ghostty_terminal_finish_block(
        terminal: Terminal,
        out_handle: *mut BlockHandle,
    ) -> Result::Type;
}
unsafe extern "C" {
    #[doc = " Remove and destroy all finished blocks (e.g. a user-initiated clear).\n Does not touch the active screen.\n\n"]
    pub fn ghostty_terminal_clear_blocks(terminal: Terminal);
}
unsafe extern "C" {
    #[doc = " Remove and destroy one finished block.\n\n"]
    pub fn ghostty_terminal_remove_block(terminal: Terminal, handle: BlockHandle) -> Result::Type;
}
unsafe extern "C" {
    #[doc = " The number of finished blocks.\n\n"]
    pub fn ghostty_terminal_block_count(terminal: Terminal) -> usize;
}
unsafe extern "C" {
    #[doc = " The handle of the block at the given index, oldest first.\n\n"]
    pub fn ghostty_terminal_block_at(
        terminal: Terminal,
        index: usize,
        out_handle: *mut BlockHandle,
    ) -> Result::Type;
...
unsafe extern "C" {
    #[doc = " The column count the block was frozen at. Blocks keep their frozen\n width, which can differ from the live terminal width after a resize.\n\n"]
    pub fn ghostty_terminal_block_cols(
        terminal: Terminal,
        handle: BlockHandle,
        out_cols: *mut u16,
    ) -> Result::Type;
...
unsafe extern "C" {
    #[doc = " The memory retained by a block's page storage in bytes (page allocations\n only; Kitty image bytes are budgeted separately).\n\n"]
    pub fn ghostty_terminal_block_bytes(
        terminal: Terminal,
        handle: BlockHandle,
        out_bytes: *mut usize,
    ) -> Result::Type;
...
#[doc = " An acquired read reference to a finished block. Obtain with\n ghostty_terminal_block_acquire(), release with\n ghostty_block_ref_release(). Usable from any thread; see the thread\n contract above.\n"]
pub type BlockRef = *mut ::std::os::raw::c_void;
unsafe extern "C" {
    #[doc = " Take a read reference on a finished block. ANY THREAD.\n\n The reference pins an immutable snapshot readable without locks via the\n ghostty_block_ref_* functions.\n\n         or while the writer is reflowing the block (retry next frame)\n"]
    pub fn ghostty_terminal_block_acquire(
        terminal: Terminal,
        handle: BlockHandle,
        out_ref: *mut BlockRef,
    ) -> Result::Type;
}
unsafe extern "C" {
    #[doc = " Release a reference from ghostty_terminal_block_acquire(). ANY THREAD.\n Frees the block if it was removed/evicted and this was the last\n reference. NULL is a no-op.\n\n"]
    pub fn ghostty_block_ref_release(ref_: BlockRef);
}
unsafe extern "C" {
    #[doc = " The (id, generation) of the acquired snapshot — the stable cache key\n for shaped/rendered rows. ANY THREAD (holding the ref).\n"]
    pub fn ghostty_block_ref_handle(ref_: BlockRef, out_handle: *mut BlockHandle) -> Result::Type;
}
unsafe extern "C" {
    #[doc = " Logical row count of the snapshot. ANY THREAD (holding the ref).\n"]
    pub fn ghostty_block_ref_row_count(ref_: BlockRef, out_rows: *mut usize) -> Result::Type;
}
unsafe extern "C" {
    #[doc = " Column count of the snapshot. ANY THREAD (holding the ref).\n"]
    pub fn ghostty_block_ref_cols(ref_: BlockRef, out_cols: *mut u16) -> Result::Type;
}
unsafe extern "C" {
    #[doc = " Page-storage bytes of the snapshot. ANY THREAD (holding the ref).\n"]
    pub fn ghostty_block_ref_bytes(ref_: BlockRef, out_bytes: *mut usize) -> Result::Type;
}
unsafe extern "C" {
    #[doc = " Resolve a row of the snapshot to a grid reference at x=0. All grid-ref\n readers work on the result; it is valid while the reference is held.\n ANY THREAD (holding the ref).\n\n         of range\n"]
    pub fn ghostty_block_ref_grid_ref(
        ref_: BlockRef,
        row: usize,
        out_ref: *mut GridRef,
    ) -> Result::Type;
...
#[doc = " An inclusive cell range of a block for text export.\n\n This is a sized struct. Use GHOSTTY_INIT_SIZED() to initialize it.\n"]
#[repr(C)]
#[derive(Debug, Default, Copy, Clone)]
pub struct BlockFormatOptions {
    ...
    pub br_row: usize,
    pub br_col: u16,
    #[doc = " Rejoin soft-wrapped lines (no newline at a wrap point)."]
    pub unwrap: bool,
...
#[allow(clippy::unnecessary_operation, clippy::identity_op)]
const _: () = {
    ["Size of BlockFormatOptions"][::std::mem::size_of::<BlockFormatOptions>() - 40usize];
    ["Alignment of BlockFormatOptions"][::std::mem::align_of::<BlockFormatOptions>() - 8usize];
    ["Offset of field: BlockFormatOptions::size"]
        [::std::mem::offset_of!(BlockFormatOptions, size) - 0usize];
    ["Offset of field: BlockFormatOptions::tl_row"]
        [::std::mem::offset_of!(BlockFormatOptions, tl_row) - 8usize];
    ["Offset of field: BlockFormatOptions::tl_col"]
        [::std::mem::offset_of!(BlockFormatOptions, tl_col) - 16usize];
    ["Offset of field: BlockFormatOptions::br_row"]
        [::std::mem::offset_of!(BlockFormatOptions, br_row) - 24usize];
    ["Offset of field: BlockFormatOptions::br_col"]
        [::std::mem::offset_of!(BlockFormatOptions, br_col) - 32usize];
    ["Offset of field: BlockFormatOptions::unwrap"]
...
unsafe extern "C" {
    ...
    pub fn ghostty_block_ref_format_alloc(
        ref_: BlockRef,
        allocator: *const Allocator,
